...
<style>
.font-awesome-selectbox {
	position: relative;
}

.font-awesome-selectbox .font-awesome-icon {}

.font-awesome-selectbox .icons-selectbox {
	position: absolute;
    width:440px;
    height:0px;
    visibility: hidden;
...
.font-awesome-selectbox .icons-selectbox.show {
	height:340px;
	visibility: visible;
	opacity: 1;
}
.font-awesome-selectbox .icons-selectbox li {
    width:30px;
    height:30px;
...
.font-awesome-selectbox .icons-selectbox li i.fa {
    line-height:30px;
}
.font-awesome-selectbox .icons-selectbox li.selected,
.font-awesome-selectbox .icons-selectbox li:hover {
    color:#fff;
}
</style>
<form action="">
<div class="font-awesome-selectbox">
	<button class="icon-show">ICON</button>
	<input type="hidden" id="icons" class="font-awesome-icon" name="icon"/>
	<ul class="icons-selectbox">
		<?php
		$icon_list  = '';
		foreach($icons as $icon) {
			$icon_list .= '<li data-icon="'.$icon.'"><i class="fa '.$icon.' fa-lg"></i></li>';
		}
		echo $icon_list;
		?>
	</ul>
...
<script>
	(function($) {
		$('.icon-show').on('click', function(event) {
			...
			var font = $(this).parent('.font-awesome-selectbox');
			if (font.find('.icons-selectbox').hasClass('show')) {
				font.find('.icons-selectbox').removeClass('show');
			} else {
				font.find('.icons-selectbox').addClass('show');
			}
		});
		
		$('.icons-selectbox').on('click', 'li', function(event) {
			event.preventDefault();
			var font = $(this).parents('.font-awesome-selectbox');

			$('.icons-selectbox').find('li').removeClass('selected');
			$(this).addClass('selected');
			
			var icon = $(this).data('icon');
			
			font.find('#icons').val(icon);
			
			font.find('.icons-selectbox').removeClass('show');

			font.find('.icon-show').html('ICON <i class="fa fa-lg '+ icon +'"></i>');
		});
	})(window.jQuery);
</script>